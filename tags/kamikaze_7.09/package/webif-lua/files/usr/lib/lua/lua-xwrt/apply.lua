...
function exe_list(list)
  ...
  if list then
    for command, msg in pairs(list) do
      str = str .. msg..end_line..command..end_line
...
function init_list(list)
  ...
  for i,v in pairsByKeys(tuno) do
    tres[#tres+1]=v
  end
  
  for i,v in pairsByKeys(tdos) do
    tres[#tres+1]=i
  end
  local end_line = "\n"
  local str = ""
...
function call_parser(file,parsers_list,depends_list,exe_before,exe_after,reboot_list)
    require("/usr/lib/lua/lua-xwrt/applys/"..file)
