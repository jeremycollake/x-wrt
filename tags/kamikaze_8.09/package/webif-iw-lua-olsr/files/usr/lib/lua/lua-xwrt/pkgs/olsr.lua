...
local ifwifi = uci.get_type("wireless","wifi-iface")
  ...
  if molsr == nil then
    ...
    uci.set("olsr","system","websettings")
    uci.save("olsr")
  end  
...
--  local host = "host"..string.gsub(ip,"(%d+)%.(%d+)%.(%d+)%.(%d+)","%4\.%1\-%2\-%3")
  ...
  uci.set("olsr","nameservice","resolv_file","/etc/resolv.conf")
  uci.set("olsr","nameservice","sighup_pid_file","/var/run/dnsmasq.pid")
...
function set_menu()
  ...
  __MENU.HotSpot.OLSR:Add("Status")
  __MENU.HotSpot.OLSR.Status = menuClass.new()
  __MENU.HotSpot.OLSR.Status:Add("Links","olsr.sh?option=Links")
  __MENU.HotSpot.OLSR.Status:Add("Neighbors","olsr.sh?option=Neighbors")
  __MENU.HotSpot.OLSR.Status:Add("Topology","olsr.sh?option=Topology")
  __MENU.HotSpot.OLSR.Status:Add("HNA","olsr.sh?option=HNA")
  __MENU.HotSpot.OLSR.Status:Add("MID","olsr.sh?option=MID")
  __MENU.HotSpot.OLSR.Status:Add("Routes","olsr.sh?option=Routes")
