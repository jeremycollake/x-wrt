...
helpitem|HotSpot Debug
helptext|Helptext HotSpot Debug#Enable/Disable debugging
field|@TR<<DHCP Network>>|chilli_net
text|chilli_net|$FORM_chilli_net
helpitem|HotSpot DHCP Network
helptext|Helptext HotSpot DHCP Netwok#Client's DHCP Network IP Subnet (192.168.182.0/24 by default)
...
helpitem|HotSpot DHCP Lease
helptext|Helptext HotSpot DHCP Lease#DHCP Lease time for clients before expires (default is 600)
...
helpitem|HotSpot DHCP DNS
helptext|Helptext HotSpot DHCP DNS#DNS Servers offered to clients (if omitted system default will be used)
...
helpitem|HotSpot Pidfile
helptext|Helptext HotSpot Pidfile#File to store information about the process id
field|@TR<<Dynamic IP Pool>>|chilli_dynip
text|chilli_dynip|$FORM_chilli_dynip
helpitem|HotSpot Dynamic IP Pool
helptext|Helptext HotSpot Dynamic IP Pool#Allocation of dynamic IP Addresses to clients
field|@TR<<Static IP Pool>>|chilli_statip
text|chilli_statip|$FORM_chilli_statip
helpitem|HotSpot Static IP Pool
helptext|Helptext HotSpot Static IP Pool#Allocation of static IP Addresses 
end_form
EOF
